...
def perform_update(progress_callback=None):
    ...
    try:
        ...
        ps_command = (
            ...
            f"    if ($_.Name -eq 'app') {{ "
            f"      # Navegamos dentro de 'app' "
            f"      Get-ChildItem -Path $_.FullName | ForEach-Object {{ "
            f"        if ($_.Name -eq 'data') {{ "
            f"          # DENTRO DE DATA: Borramos todo menos session.json "
            f"          Get-ChildItem -Path $_.FullName | Where-Object {{ $_.Name -ne 'session.json' }} | Remove-Item -Recurse -Force "
            f"        }} elseif ($_.Name -ne 'logs') {{ "
            f"          # FUERA DE DATA: Borramos todo menos logs "
            f"          Remove-Item -Path $_.FullName -Recurse -Force "
            f"        }} "
            f"      }} "
