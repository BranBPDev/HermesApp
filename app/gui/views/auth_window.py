...
from app.gui.styles.styles import COLOR_BG_DARK, COLOR_BG_SIDE, FONT_TITLE, COLOR_PRIMARY
...
class AuthView(tk.Frame):
    ...
    def _render(self, event=None):
        ...
        # Panel Derecho (Formulario visual)
        cx = (w * 0.75)
        # Usamos nuestros componentes del esqueleto
        CInput(self.canvas, cx-150, h/2-60, 300, 45, "USUARIO").draw()
        CInput(self.canvas, cx-150, h/2+20, 300, 45, "CONTRASEÑA").draw()
        
        btn_login = CButton(self.canvas, cx-150, h/2+100, 300, 50, "ENTRAR", self._handle_auth)
        btn_login.draw()

    def _handle_auth(self):
        # Aquí iría la lógica de recolección de datos
        self.on_success()